...
/**
 * All utils for Intent. Intent for send email, sms... <br></br>
...
object IntentUtils {
     ...
     */
    @JvmOverloads fun openPlayStore(context: Context, openInBrowser: Boolean = true): Intent? {
     ...
     * *                  [.showLocation]
     */
    fun showStreetView(latitude: Float, longitude: Float,
                       yaw: Float?, pitch: Int?, zoom: Float?, mapZoom: Int?): Intent {
        val builder = StringBuilder("google.streetview:cbll=")
                .append(latitude).append(",").append(longitude)
        if (yaw != null || pitch != null || zoom != null) {
            val cbpParam = String.format("%s,,%s,%s",
                    yaw ?: "", pitch ?: "",
                    zoom ?: "")
     ...
     * @see .findLocation
     */
    fun showLocation(latitude: Float, longitude: Float,
                     zoomLevel: Int?): Intent {
        val intent = Intent()
        intent.action = Intent.ACTION_VIEW
     ...
     * @return
     */
    fun showLocationWithMarker(latitude: Float, longitude: Float,
                               zoomLevel: Int?, label: String): Intent {
        val intent = Intent()
        intent.action = Intent.ACTION_VIEW
        var data = String.format("geo:%s,%s?q=%s,%s(%s)", latitude, longitude, latitude, longitude, label)
        if (zoomLevel != null) {
            data = String.format("%s?z=%s", data, zoomLevel)
        }
        intent.data = Uri.parse(data)
     ...
     * @return
     */
    fun showLocationWithBrowser(latitude: Float, longitude: Float): Intent {
        val intent = Intent()
        intent.action = Intent.ACTION_VIEW
        val data = String.format("http://maps.google.com/maps?q=%s,%s&iwloc=A&hl=es", latitude, longitude)
        intent.data = Uri.parse(data)
        return intent
    }
     ...
     * Telephone number normalization described for [.callPhone]
     ...
     * *                sizes differs
     * *
     * @return Intent with `data`-extra in
     * * `onActivityResult` which contains result as a
     * * [android.graphics.Bitmap]. See demo app for details
     */
    fun cropImage(context: Context, image: File, outputX: Int,
                  outputY: Int, aspectX: Int, aspectY: Int, scale: Boolean): Intent {
        val intent = Intent("com.android.camera.action.CROP")
        intent.type = "image/*"
